...
res_pe = DataFrame(
    CSV.File(
        joinpath(@__DIR__, "results", "Results bioequivalence homogeneous sas sw ml.csv")
    ),
)
...
res_tests_sw = DataFrame(
    CSV.File(
        joinpath(
            @__DIR__, "results", "Results bioequivalence homogeneous sas tests3 sw ml.csv"
        ),
    ),
)
...
sas_asycov_df = DataFrame(
    CSV.File(
        joinpath(
            @__DIR__, "results", "Results bioequivalence homogeneous sas asycov ml.csv"
        ),
    ),
)
