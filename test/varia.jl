...
@test_throws ErrorException small_sample_adjust(m, Satterthwaite(fim=ObservedFIM()))
@test_throws ErrorException small_sample_ftest(m, ones(2, 1); method=KenwardRoger())
@test_throws ErrorException small_sample_ftest(m, ones(2, 1); method=Satterthwaite())
